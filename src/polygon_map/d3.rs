...
impl D3Color {
    ...
    pub fn interpolate(&self, other: &D3Color, dist: F) -> Self {
        assert!(dist >= 0.0.into());
        assert!(dist <= 1.0.into());
        Self::new_rgba(
            self.r + (*((f(other.r) - f(self.r)) * dist)) as u8,
            self.g + (*((f(other.g) - f(self.g)) * dist)) as u8,
            self.b + (*((f(other.b) - f(self.b)) * dist)) as u8,
            self.a + (*((f(other.a) - f(self.a)) * dist)) as f32
        )
    }
