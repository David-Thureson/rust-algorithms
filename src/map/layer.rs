...
impl Layer {
    ...
    pub fn take_to_pre_water_state(&mut self, goal_state: &LayerState, force_new: bool) -> bool {
        ...
        debug_assert_eq!(goal_state.point_count, self.layer_state.point_count);
        debug_assert_eq!(goal_state.polygons, self.layer_state.polygons);
        // It's possible that the caller asked for a lower relax state in which case we did
        // nothing.
        debug_assert!(goal_state.relax_level <= self.layer_state.relax_level);
        debug_assert_eq!(goal_state.vertices_and_edges, self.layer_state.vertices_and_edges);
    ...
    pub fn add_voronoi_polygons(&mut self) {
        ...
        // debug_assert_eq!(vor_polys.len(), self.vor_points.len());
        //bg!(&self.vor_points);
    ...
    pub fn set_water_from_pct(&mut self, pct: F) {
        ...
        // debug_assert!(self.layer_type == LayerType::OceanGuide);
    ...
    pub fn clear_water(&mut self) {
        ...
        // debug_assert!(self.layer_type == LayerType::Terrain);
        for polygon in self.polygons.iter_mut() {
    ...
    pub fn gen_vertices_and_edges(&mut self) {
        ...
        for (index, polygon) in self.polygons.iter().enumerate() {
            debug_assert_eq!(index, polygon.sequence);
        }
        ...
        for polygon in self.polygons.iter_mut() {
            ...
            for point in polygon.points.iter_mut() {
                ...
                debug_assert!(!self.vertices[vertex_index].polygons.contains(&polygon.sequence));
                self.vertices[vertex_index].polygons.push(polygon.sequence);
                polygon.vertex_indexes.push(vertex_index);
            }
            debug_assert_eq!(polygon.vertex_indexes.len(), polygon.points.len());
            ...
            for vertex_index_pair in polygon.vertex_index_ordered_pairs() {
                ...
                let edge_index = if edge_map.contains_key(&edge_key) {
                    *edge_map.get(&edge_key).unwrap()
                    ...
                    // The edge already has its list of vertices from its constructor. Now let the
                    // two vertices know about the edge.
                    for vertex_index in vertex_index_pair.iter() {
                        debug_assert!(!self.vertices[*vertex_index].edges.contains(&next_edge_sequence));
                        self.vertices[*vertex_index].edges.push(next_edge_sequence);
                    }

                    debug_assert!(!self.edges.contains(&new_edge));
                    self.edges.push(new_edge);
                    next_edge_sequence += 1;
                    next_edge_sequence - 1
                };
                debug_assert!(!self.edges[edge_index].polygons.contains(&polygon.sequence));
                self.edges[edge_index].polygons.push(polygon.sequence);
                // An edge may be attached either to one or two polygons.
                debug_assert!(self.edges[edge_index].polygons.len() <= 2);
                polygon.edge_indexes.push(edge_index);
            }
            debug_assert_eq!(polygon.edge_indexes.len(), polygon.points.len());
        }
        // Make sure the sequence numbers for vertices match their place in the vector.
        for (index, vertex) in self.vertices.iter().enumerate() {
            debug_assert_eq!(index, vertex.sequence);
        }
        // Make sure the sequence numbers for edges match their place in the vector.
        for (index, edge) in self.edges.iter().enumerate() {
            debug_assert_eq!(index, edge.sequence);
            debug_assert!(edge.polygons.len() >= 1);
            debug_assert!(edge.polygons.len() <= 2);
        }
    ...
    pub fn calc_distance_to_water(&mut self, dist_mult: F, dist_add: F, short_circuit: bool) {
        for polygon in self.polygons.iter_mut() {
            polygon.altitude = None;

            //bg!(&polygon);
            //bg!(polygon.polygon_type == PolygonType::Ocean);
        }
        for vertex in self.vertices.iter_mut() {
            vertex.vertex_type = VertexType::Unknown;
            vertex.steps_to_water = None;
        ...
        for vertex_index in 0..self.vertices.len() {
            ...
            if ocean_count > 0 {
                ...
                vertex.vertex_type = if vertex.polygons.len() == ocean_count {
                    VertexType::Ocean
                } else {
                    VertexType::Coast
                };
                ...
                let mut trav = VertexStepDistanceTraverser::new(dist_mult, dist_add, vertex_index);
                //bg!(&trav);
                // let mut trav_count = 0;
                while let Some(item_step_distance) = trav.next(self, short_circuit) {
                    //bg!(&item_step_distance);
                    //trav_count += 1;
                    let one_vertex_index = item_step_distance.index;
                    if item_step_distance.complete || self.vertex_ocean_count(one_vertex_index) > 0 {
                        // We've reached a vertex that touches the ocean.
                        let this_vertex =self.vertices.get_mut(vertex_index).unwrap();
                        this_vertex.vertex_type = VertexType::Land;
                        this_vertex.steps_to_water = Some(item_step_distance.step_count);
                        this_vertex.distance_to_water = Some(item_step_distance.dist);
                        //bg!(trav_count);
                        //bg!(this_vertex);
                        break;
                    }
